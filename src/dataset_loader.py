...
def find_combined_images(root: str | Path, split: str | None = None, source: str | None = "dataset") -> list[SampleItem]:
    ...
    for search_root, source_name, split_name in search_roots:
        ...
        for combined_path in sorted(search_root.rglob("*_combined.jpg")):
            ...
            items.append(SampleItem(stem, combined_path, parent / f"{stem}_combined.json", parent / f"{stem}_left{suffix}", parent / f"{stem}_right{suffix}", split_name, source_name, relative_folder, error_type))
...
def find_items_from_review_csv(csv_path: str | Path, dataset_root: str | Path | None = None) -> list[SampleItem]:
    ...
    local_root = Path(dataset_root) if dataset_root else (Path(env_root) if env_root else None)
    ...
    for row_index, row in df.reset_index(drop=True).iterrows():
        ...
        json_path = Path(clean_value(row.get("json_path"))) if clean_value(row.get("json_path")) else parent / f"{stem}_combined.json"
        left_path = Path(clean_value(row.get("left_image_path"))) if clean_value(row.get("left_image_path")) else parent / f"{stem}_left{suffix}"
        right_path = Path(clean_value(row.get("right_image_path"))) if clean_value(row.get("right_image_path")) else parent / f"{stem}_right{suffix}"
        if local_root is not None and (not image_path_text or not image_path.exists()):
            image_path, json_path, left_path, right_path = _local_paths(local_root, source, split, relative_folder, image_id)
        ...
        items.append(SampleItem(image_id, image_path, json_path, left_path, right_path, split, source, relative_folder, error_type, llm_info))
    return items


def row_to_llm_info(row: dict[str, Any], row_index: int, csv_path: Path) -> dict[str, Any]:
    interesting_keys = ["llm_change", "llm_class", "confidence", "review_required", "review_required_final", "review_reasons", "label_mismatch", "detail_mismatch", "detail_mismatch_keys", "low_confidence", "empty_class_when_change", "priority_score", "reason_ko", "reason_en", "raw_response", "error", "llm_provider", "llm_model", "prompt_path", "batch_id", "run_id", "work_mode", "attempt_count", "input_tokens", "output_tokens", "total_tokens", "estimated_cost_usd", "processed_at", "original_change", "original_reason", "original_arti", "original_arti_bu", "original_arti_bu_t", "original_arti_binil", "original_arti_road", "original_arti_roa_m", "original_arti_other", "original_tree", "original_fore", "original_farm", "original_water", "arti", "arti_bu", "arti_bu_t", "arti_binil", "arti_road", "arti_roa_m", "arti_other", "tree", "fore", "farm", "water"]
...
def infer_relative_folder_from_path(path: Path, split: str, source: str) -> str:
    parts = list(path.parts); lower_parts = [part.lower() for part in parts]
    try:
        if source == "errors" and "errors" in lower_parts and split in lower_parts:
            start = lower_parts.index(split) + 1; rel_parts = parts[start:-1]; return "/".join(rel_parts) if rel_parts else "."
...
def resolve_search_roots(root_path: Path, split: str, source: str) -> list[tuple[Path, str, str]]:
    source_names = list(SOURCES) if source == "both" else [source]; split_names = list(SPLITS) if split == "all" else [split]
    results: list[tuple[Path, str, str]] = []; seen: set[Path] = set()
    for source_name in source_names:
        if source_name not in SOURCES: continue
        for split_name in split_names:
            if split_name not in SPLITS: continue
            for path in candidate_split_paths(root_path, source_name, split_name):
                resolved = path.resolve()
                if path.exists() and resolved not in seen: results.append((path, source_name, split_name)); seen.add(resolved)
    return results


def candidate_split_paths(root_path: Path, source: str, split: str) -> list[Path]:
    candidates: list[Path] = []
    if source == "dataset": candidates.extend([root_path / "dataset" / split, root_path / split])
    elif source == "errors": candidates.extend([root_path / "errors" / split, root_path / "dataset" / "errors" / split, root_path / split])
    if root_path.name.lower() == split:
        parent_name = root_path.parent.name.lower(); grand_parent_name = root_path.parent.parent.name.lower() if root_path.parent.parent else ""
        if source == "dataset" and parent_name == "dataset": candidates.append(root_path)
        if source == "errors" and (parent_name == "errors" or grand_parent_name == "errors"): candidates.append(root_path)
    if source == "errors" and split in [part.lower() for part in root_path.parts] and "errors" in [part.lower() for part in root_path.parts]: candidates.append(root_path)
    return candidates


def get_relative_folder(parent: Path, search_root: Path) -> str:
    try: rel = parent.relative_to(search_root)
    except ValueError: return "."
    text = rel.as_posix(); return text if text != "." else "."


def infer_error_type(relative_folder: str) -> str:
    return "" if not relative_folder or relative_folder == "." else relative_folder.split("/")[0]


def infer_split(path: Path) -> str:
    parts = [p.lower() for p in path.parts]
    for split in SPLITS:
        if split in parts: return split
    return "unknown"


def infer_source(path: Path) -> str:
    parts = [p.lower() for p in path.parts]
    if "errors" in parts: return "errors"
    if "dataset" in parts: return "dataset"
    return "unknown"
