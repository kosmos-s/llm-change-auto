"""Gemini-first Streamlit UI for the auto-labeling pipeline."""
...
def show_csv_preview(path: Path, title: str, max_rows: int = 20) -> None:
    ...
    st.download_button(f"{path.name} 다운로드", csv_bytes, file_name=path.name, mime="text/csv", use_container_width=True)
...
def make_default_prefix(source: str, split: str, limit: int) -> str:
    ...
    return f"gemini_{source}_{split}_{limit_part}"


def render_settings() -> dict[str, object]:
    st.sidebar.title("Gemini 자동화 설정")
    ...
    model = st.sidebar.text_input("Gemini 모델", value="gemini-2.5-flash")
...
def main() -> None:
    ...
    st.title("Gemini 자동화 파이프라인")
    st.caption("데이터 인덱스 생성 → Gemini 자동판별 → 기존 JSON 라벨 비교 → 검수 대상 CSV 생성")
    ...
    st.markdown("## 2. Gemini 자동판별")
    c1, c2 = st.columns([1, 3])
    with c1:
        if st.button("Gemini 실행", type="primary", use_container_width=True):
            if not index_path.exists():
                st.error("먼저 dataset_index.csv를 생성하세요.")
            elif not prompt_path.exists():
                st.error(f"프롬프트 파일이 없습니다: {prompt_path}")
            else:
                with st.spinner("Gemini 자동판별 실행 중... 10장도 시간이 걸릴 수 있습니다."):
                    run_llm(
                ...
                st.success(f"Gemini 결과 생성 완료: {llm_path}")
    with c2:
        st.code(
            "python src\\run_llm_labeling.py "
            f"--provider gemini --model {settings['model']} "
    ...
    show_csv_preview(llm_path, "Gemini 결과 미리보기")
    ...
    with c1:
        if st.button("비교 실행", type="primary", use_container_width=True):
            if not llm_path.exists():
                st.error("먼저 Gemini 결과 CSV를 생성하세요.")
            else:
                with st.spinner("기존 라벨과 Gemini 결과 비교 중..."):
    ...
    st.markdown("## 다음 단계")
    st.info("검수 대상 CSV가 만들어지면, 검수 UI에서 LLM 검수 대상 CSV 모드로 해당 파일만 순서대로 확인하세요.")
